...
where, $g$ and $f$ are defined as follows:
 ...
 The edge $j$ appears in the final consensus network if the reported edges is greater than the threshold:
  ...
  \subsubsection*{Notation}
  \vspace{-5mm}
  This section defines notation for the network variability analysis in our study.

  $W$, the number of co-occurrence networks generated from all possible combinations of tools and parameters in the workflow. Note that this is different from $w$, which counted only the possible network inference modules.

  $Q$, the number of combined nodes from all $W$ networks

  $N^i$ = The weights of the $i^{th}$ co-occurrence network represented as a $Q \times Q$ adjacency matrix, where $i \in {1, \dots, W}$

  $N^i_{a,b}$ = edge $(a,b)$ in network $i$

  $\bar{N}_i$ is a "flattened" version of the adjacency matrix $N^i$ into a $Q^2 \times 1$ column vector, where all columns are stacked onto each other into a $Q^2$ long vector:
  ...
  \vspace{-5mm}
   ...
   $\bar{N}^i_j$ is assigned a value of 0 if edge $j$ did not exist in network $i$ but was present in other networks.
   ...
   \mathbf{\bar{N}} =
     \begin{blockarray}{*{4}{c} l}
      \begin{block}{*{4}{>{$\footnotesize}c<{$}} l}
        $\bar{N}$^1 & $\bar{N}$^2 & \dots & $\bar{N}$^{W} & \\
      \end{block}
      \begin{block}{[*{4}{c}]>{$\footnotesize}l<{$}}
       n^1_1 & n^2_1 & \cdots  & n^{W}_1 \bigstrut[t]& $\bar{N}$_1 \\
       n^1_2 & n^2_2 & \cdots  & n^{W}_2 & $\bar{N}$_2 \\
       \vdots & \vdots & \vdots  & \vdots & \vdots \\
       n^1_{Q^2} & n^2_{Q^2} & \cdots  & n^{W}_{Q^2} & $\bar{N}$_{Q^2}
      \end{block}
     \end{blockarray}
  \end{equation*}
  ...
  In order to remedy this issue, we perform Principal Component Analysis on the matrix $\mathbf{\bar{N}}$ to obtain the $\mathbf{C}$ matrix ($W \times c$) of components for each network, such that we reduce the dimensions from the $Q^2$ dimensional edge space to a $c$ dimensional component space.
  ...
  In particular, we infer parameters $\alpha_i$ such that:
   ...
   The variance contributed by each step of the pipeline is calculated for every component in $\mathbf{C}$ matrix through ANOVA using the Python \texttt{statsmodels}~\cite{seaboldStatsmodelsEconometricStatistical2010} package and is shown in Figure~\ref{fig:figure2}A.
