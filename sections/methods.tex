...
\section*{Materials and Methods}
  ...
  Sequences from both control (212 samples including neurotypical and donors) and autism spectrum disorder (ASD) (126 samples) patients were included in the analyses, but the groups were analyzed independently in the pipeline after the \ac{op} step.
  All the network analyses in the study, unless explicitly mentioned, are performed indeed on the healthy and ASD samples in the fecal microbiome transplant study.
  ...
  The expected sequences from the mock communities are trimmed to the V4 region before being subject to UniFrac analyses.
...
where, $g$ and $f$ are defined as follows:
 ...
 The edge $j$ is selected to be present in the final consensus network if the number of networks in which $j$ appears is greater than a threshold, i.e:
  ...
  Finally, for each dataset we evaluated the precision of the associations predicted by the individual network inference methods as well as the consensus (Figure \ref{fig:figure5}).
