...
\section*{Materials and Methods}
 ...
 % TODO: Use bold for matrices and lower-case bold for vectors
  This section defines the notation for the consensus network component of our pipeline, which assumes that all networks to be compared have the same number of nodes.

  $w$ = the number of co-occurrence network algorithms

  $q$ = the number of combined nodes from all networks in $w$


  $N^i$ = The weights of the $i^{th}$ co-occurrence network represented as a $q \times q$ adjacency matrix, where $i \in {1, w}$

  $P^i$ = The pvalues of the $i^{th}$ co-occurrence network represented as a $q \times q$ adjacency matrix, where $i \in {1, q}$

  $N^i_{a,b}$ = edge $(a,b)$ in network $i$

  $\bar{N}^i$ is a "flattened" version of the adjacency matrix $N^i$ into a $q^2 \times 1$ column vector, where all columns are stacked onto each other into a $q^2$ long vector:
  ...
  For all correlation-based methods $k \in 1:w$, $1000$ permutations of the original \ac{otu} counts data were generated~\cite{Watts2018}.
  ...
  This step is performed using the Brown's p-value merging method~\cite{Poole_Gibbs_Shmulevich_Bernard_Knijnenburg_2016,faustCoNetAppInference2016} (refer Supplementary).
  ...
  \end{equation}
  where, $\hat{P}_j$ is the combined p-value for the edge $j$
  ...
  For each pair of nodes, we determine the number of network inference methods that report an edge $j=k$ between them, i.e. $\bar{N}^{i}_{j=k}, \forall i \in 1:w$.
  Each node-pair will have an edge in the final consensus network if the number of reported edges is more than the threshold (Equation~\ref{eqn:simple-voting2})

 For each edge $j$, we obtain $M_j$ which represents the number of networks in which edge $j$ is reported, i.e.:
  ...
  where,
  $w$ is the number of network inference methods used in the consensus,
  \begin{equation*}
    g(x) =
    \begin{cases}
       & 0 \text{ if } x=0, \\
  ...
  This algorithm performs a consensus based on the sum of all edges reported between a node-pair. First, the network generated by each network inference method is re-scaled so that $\max_{j}(\mid \bar{N}^i_j \mid) = 1$ (Equation~\ref{eqn:scaled-sum-rescaling})


  \begin{equation}
    S^{i} = \frac{\bar{N}^{i}}{\max(\mid \bar{N}^{i} \mid)}
    \label{eqn:scaled-sum-rescaling}
  \end{equation}

  The re-scaling is performed because the associations reported by the direct association metrics are not necessarily correlations and are not directly comparable to the associations from the correlation-based methods due to their lower weights.
  This problem can also be alleviated by weighing each network inference method differently, but for simplicity we chose the current approach.
  Next, for each edge $j$, we sum the weights of all reported edges from the different networks (Equation \ref{eqn:scaled-sum}).

  \begin{equation}
    s_j = \left\mid \sum_i S^i_j \right\mid
    \label{eqn:scaled-sum}
  \end{equation}


  An edge $j$ will exist between a given node-pair in the consensus network if:
  \begin{equation}
    s_j > (w - 1) \times p
  ...
  The advantage of this method is that it also takes into account the strength of the association reported for that particular node in the inferred networks.
  ...
  $w^\prime$ = the number of co-occurrence networks generated from all possible combinations of tools and parameters in the workflow.

  $q^\prime$ = the number of combined nodes from all networks in $w^\prime$
 ...
 First, we obtain co-occurrence network $\bar{N}_i$ for each possible tool and parameter combination in the workflow.
 Then, we obtain the merged matrix $\mathbf{\bar{N}}$ with $q^{\prime2}$ edges from $w^\prime$ networks, where, each column $i$ is the vector representation of one of the networks $\bar{N}^i$, each row $j$ is the vector representation of one particular edge in all networks $\bar{N}_j$ (assigned a value of 0 if the edge did not exist in the network but in other networks), and each element $n^i_j$ belongs to edge $j$ and network $i$.
  ...
  To infer the variability contributed due to different steps in the pipeline we perform a linear regression on each edge in $\mathbf{\bar{N}}$ and a subsequent ANOVA to extract the group variances.
  One issue with this approach is that the possible correlations between the edges of the network could lead to inaccurate estimates of the variance if a GLM were used to directly model the relationships between edges and steps in the workflow.
