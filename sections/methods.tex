...
\section*{Materials and Methods}
 ...
 We then merged the networks generated from all possible combinations of tools into a table (N, see below) in which each column represents one network.
  ...
  In other words, $N$ is the merged table, each column $N_i$ is the vector representation of one of the networks, and each row $L_i$ represents the one particular edge in all networks (assigned 0 if the edge does not exist in the network).
