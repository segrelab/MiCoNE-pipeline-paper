...
\section*{Results}
  ...
  % TODO: Figure out the how the OP step plays a role in the PCA plot
  % TODO: Supplementary Figure S2
  ...
  % TODO: Supplementary work - Figure S3
  ...
  % TODO: Update thresholds (10) on the mock datasets and check again
  ...
  % TODO: Annotate Figure 4A with bacteria names
 ...
 % There are also other studies, [ref, OTT paper] that focus on the database aspect and they too have found that...
  ...
  % TODO: Update after getting SparCC results
  In order to obtain a quantitative picture of the differences between the inferred networks, we analyzed the distribution of common nodes and edges (Figure~\ref{fig:figure5}B) using UpSet plots~\cite{Lex}.
  ...
  % TODO: What is the final concluding statement here? How to segway to the next section

  \FloatBarrier

  \subsubsection*{The consensus network}

  Therefore, we propose the construction of a consensus network (Figure~\ref{fig:figure5}C) involving \ac{mldm}, \ac{spieceasi} and \ac{sparcc}. This consensus network is built by merging the p-values generated from bootstraps of the original taxonomy counts table using the Browns p-value combining method~\cite{brown_400_1975} (see Methods section).

  % FIXME: We have synthetic data
  Similar to the previous steps of the pipeline, where were we evaluated the performance of methods on mock datasets, we can evaluate the network inference algorithms using synthetic interaction data.
  ...
  % TODO: Change the order of Figure 6 and 7
  ...
  The recommended tool for the \ac{dc} step (\ac{dada2}) was chosen based on their accuracy in recapitulating the reference sequences in mock communities and synthetic data.
  ...
  Finally, we make use of the consensus methods developed in this paper ... on the networks generated using \ac{mldm}, \ac{spieceasi} and \ac{sparcc} to obtain a final consensus network in the \ac{ni} step.
