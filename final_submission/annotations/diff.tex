...
%DIF DEL ../resubmission/manuscript.tex   Thu Oct 20 15:53:52 2022
%DIF ADD manuscript.tex                   Thu Mar  9 10:18:24 2023
...
Microbial communities are ubiquitous and play an important role in marine and terrestrial environments, urban ecosystems, and human health \cite{lima-mendezDeterminantsCommunityStructure2015a,Thompson2017,royo-llonchCompendium530Metagenomeassembled2021,tedersooFungalBiogeographyGlobal2014,dankoGlobalMetagenomicMap2021,mclellanMicrobiomeUrbanWaters2015,HumanMicrobiomeProjectConsortium2012}.
 ...
 In particular, a powerful tool for translating microbiome composition data into knowledge is the construction of association (co-occurrence\DIFdelbegin \DIFdel{or correlation}\DIFdelend ) networks, in which  microbial taxa are represented by nodes, and frequent co-occurrences (or negative \DIFdelbegin \DIFdel{correlations}\DIFdelend \DIFaddbegin \DIFadd{co-occurrences}\DIFaddend ) across datasets are encoded as edges between nodes.
  \DIFaddbegin \todo[noline]{R3.7}
 \DIFaddend While the relationship between directly measured interactions~\cite{lubbeExometabolomicAnalysisCrossFeeding2017,Jian2020,Hsu2019} and statistically inferred co-occurrence is still poorly understood \cite{Zuniga2017,Rottjers2018}, a significant amount of effort has gone into estimating \DIFdelbegin \DIFdel{correlations }\DIFdelend \DIFaddbegin \DIFadd{co-occurrences }\DIFaddend from large microbiome sequence datasets~\cite{faustMicrobialCooccurrenceRelationships2012,leeCrosskingdomCooccurrenceNetworks2022,faustMicrobialInteractionsNetworks2012a,maEarthMicrobialCooccurrence2020a}.
...
\section*{Results}
  ...
  Based on \DIFdelbegin \DIFdel{their slightly better performance }\DIFdelend \DIFaddbegin \DIFadd{being among the top performers }\DIFaddend on the mock datasets, their de novo error correcting nature and previous independent evaluation~\cite{Nearing2018}, \ac{dada2} and Deblur appear to be the most reliable.
  \DIFaddbegin \todo[noline]{R2.4}
  \DIFaddend This is because the open-reference and de novo clustering methods return a much larger number of \ac{otu}s compared to the other pipelines and would affect the accuracy of the network inference step if stringent filtering is not performed.
  %\todo[noline]{R2.6}
  ...
  \subsection*{NI: Different network inference methods drastically affect edge-density and connectivity}
   ...
   \DIFaddbegin \DIFadd{In general, when we refer to co-occurrences we include the associations from both correlation based methods and direct association based methods.
  }\todo[noline]{R3.7}
   \DIFadd{Whereas, correlations exclusively refer to those inferred by the correlation methods.
   }\DIFaddend Note that while Pearson and Spearman methods are included in the pipeline for completeness, they tend to generate a large number of spurious edges as they are not intended for compositional datasets.
 ...
 Inspired by previous approaches~\cite{bustinceFuzzySetsTheir2008,tsarevApplicationMajorityVoting2018}, we developed two methods that take into consideration the evidence offered by each network inference algorithm and generate a consensus network that contains the common edges among the inferred networks.
  ...
  The pipeline enables the selection of any subset of methods for the consensus calculation. Currently, by default, all direct \DIFdelbegin \DIFdel{methods }\DIFdelend \DIFaddbegin \DIFadd{association methods (}\acs{spieceasi}\DIFadd{, }\acs{cozine}\DIFadd{, }\acs{harmonies}\DIFadd{, }\acs{spring}\DIFadd{, }\acs{mldm}\DIFadd{, and FlashWeave) }\DIFaddend are used, together with \acs{sparcc} and propr.
  \DIFaddbegin \todo[noline]{R3.2}
\DIFaddend 
  ...
  The interaction network used to generate the counts table was used as the source of true interactions to calculate the precision \DIFaddbegin \DIFadd{and sensitivity }\DIFaddend (Figure \ref{fig:figure5}) \DIFdelbegin \DIFdel{and sensitivity (Figure }\DIFdelend \DIFaddbegin \DIFadd{of the consensus algorithms.
  These values are also compared with the precision and sensitivity of four individual network inference methods (two correlation-based, i.e. propr, FlashWeave, and two direct association methods, i.e. }\acs{sparcc}\DIFadd{, and }\acs{spieceasi}\DIFadd{), which were selected based on their high precision (see Figure ~}\DIFaddend \ref{fig:figure_s5} and Figure \DIFdelbegin \DIFdel{\ref{fig:figure_s6}) for each network inference algorithm}\DIFdelend \DIFaddbegin \DIFadd{~\ref{fig:figure_s6})}\DIFaddend .
  ...
  \DIFaddbegin \todo[noline]{R1}
  \todo[noline]{R2.1}
  \DIFadd{Although this increase in precision is associated with a drop in sensitivity (as the consensus parameter $\theta$ increases), the consensus networks provide valuable and practically helpful results, in the form of a short list of high-confidence associations.
  }\DIFaddend Overall, the scaled-sum method for \DIFdelbegin \DIFdel{$p=1.000$ }\DIFdelend \DIFaddbegin \DIFadd{$\theta=1.000$ }\DIFaddend performs the best (precision = $1.000$ for both NorTA and seqtime).
  \DIFaddbegin \DIFadd{Additionally, Figure \ref{fig:figure_s5} and Figure \ref{fig:figure_s6} show the precision and sensitivity values of all network inference and consensus algorithms for each interaction network in the synthetic datasets.
  }\DIFaddend The scaled-sum method for \DIFdelbegin \DIFdel{$p=0.333$ }\DIFdelend \DIFaddbegin \DIFadd{$\theta=0.333$ }\DIFaddend (default option in the pipeline) shows a high precision ($0.956$ with NorTA; $0.688$ with seqtime), without displaying \DIFaddbegin \DIFadd{a }\DIFaddend significant reduction in sensitivity (Figure~\DIFaddbegin \DIFadd{\ref{fig:figure5}, Figure~}\DIFaddend \ref{fig:figure_s5} and Figure~\ref{fig:figure_s6}).
  However, if higher precision is required \DIFdelbegin \DIFdel{$p>0.5$ }\DIFdelend \DIFaddbegin \DIFadd{$\theta>0.5$ }\DIFaddend can be considered.
\DIFdelbegin \DIFdel{Therefore, the consensus networks provide the means to obtain a short list of associations that would have a high likelihood of being present in the real association network.
}\DIFdelend 
 ...
 This was achieved by building a linear model of the edges of the network as a function of the various steps in the pipeline workflow (see Methods).
  ...
  \DIFaddbegin \DIFadd{The network variance analysis performed on the supplementary dataset (stool samples from radiation-exposed bank vole) shown in Figure~\ref{fig:figure_s8} support these observations, implying that these findings are fairly consistent across different datasets.
  }\DIFaddend These results suggest that the most important criterion for accurate comparative analysis of co-occurrence networks is the taxonomy reference database followed by the level of filtering of the taxonomy tables and the network inference algorithm used.
...
\section*{Discussion}
  ...
  Through \ac{micone}, in addition to transparently revealing the dependence of co-occurrence networks on tool and parameter choices (see Discussion in Supplementary Text for details on the DC, TA\DIFaddbegin \DIFadd{, }\DIFaddend and OP steps), we have taken advantage of our spectrum of computational options and the availability of mock and synthetic datasets, to suggest a default standard setting \DIFaddbegin \DIFadd{that streamlines comparisons across datasets}\DIFaddend .
  %\todo[noline]{R1.3}
  Additionally, we have developed a consensus approach \DIFdelbegin \DIFdel{, }\DIFdelend that can reliably generate \DIFdelbegin \DIFdel{networks that are fairly robust }\DIFdelend \DIFaddbegin \DIFadd{fairly robust networks }\DIFaddend across multiple tool choices.
  \DIFdelbegin \DIFdel{An important caveat related to these results is that due to the lack of a }\DIFdelend \DIFaddbegin \DIFadd{Even if, in the current analysis, we have shown the relevance of our approach to two very different types of microbiome datasets (human and vole stool), it is important to remember that there is no }\DIFaddend universal standard for microbial interaction data, \DIFaddbegin \DIFadd{and }\DIFaddend our conclusions are based on the specific datasets used in our analysis.
  While our analysis is based on several mock and synthetic datasets that cover a diverse range of abundance distributions and network topologies, datasets \DIFdelbegin \DIFdel{that have }\DIFdelend \DIFaddbegin \DIFadd{with }\DIFaddend drastically different distributions may require a re-assessment of the best settings.
  \DIFaddbegin \DIFadd{However, the }\ac{micone} \DIFadd{pipeline provides a platform for easy evaluation of accuracy, variance, and other properties at each workflow step for any other dataset of interest.
}\DIFaddend 
 ...
 Notably, the consensus network displays a higher precision and returns a concise list of robust associations \DIFdelbegin \DIFdel{which represent }\DIFdelend \DIFaddbegin \DIFadd{representing }\DIFaddend a valuable set for experimental validation follow-up.
  ...
  %\todo[noline]{R2.1c}
  \DIFdelbegin \DIFdel{As a consequence}\DIFdelend \DIFaddbegin \DIFadd{Consequently}\DIFaddend , two entities belonging to the same lineage where one entity is resolved to the Genus level and another is resolved to the Family level are treated as two different nodes in the network.
  Thus, \DIFdelbegin \DIFdel{the development of a metric of overlap }\DIFdelend \DIFaddbegin \DIFadd{developing an overlap metric }\DIFaddend to compare nodes with shared lineages within and across networks could enable more biologically and phylogenetically relevant comparisons.

  \DIFdelbegin \DIFdel{Although direct comparisons between }\DIFdelend \DIFaddbegin \DIFadd{In thinking about the possible biological interpretations of the }\DIFaddend co-occurrence \DIFaddbegin \DIFadd{network computed by us and others, it is important to remember that there is no solid basis for assuming that these networks carry information about physical or metabolic interactions~\mbox{%DIFAUXCMD
\cite{fisherIdentifyingKeystoneSpecies2014,pachecoFAIRRepresentationsMicrobial2022}}\hskip0pt%DIFAUXCMD
.
  Comparing co-occurrence }\DIFaddend networks and directly measured interactions \DIFdelbegin \DIFdel{are difficult to interpret and highly debated~\mbox{%DIFAUXCMD
\cite{hiranoDifficultyInferringMicrobial2019,gobernaCautionaryNotesUse2022}}\hskip0pt%DIFAUXCMD
.
  }\DIFdelend \DIFaddbegin \DIFadd{remains a major unresolved challenge~\mbox{%DIFAUXCMD
\cite{hiranoDifficultyInferringMicrobial2019,gobernaCautionaryNotesUse2022}}\hskip0pt%DIFAUXCMD
, which needs to be investigated further.
  Understanding this connection will enable the utilization of co-occurrence networks for systems in which direct interaction measurements cannot be taken.
  }\DIFaddend %\todo[noline]{R2.2}
...
\section*{Materials and Methods}
  ...
  \DIFaddbegin \DIFadd{In addition to these datasets, we have utilized a dataset containing stool samples from radiation-exposed bank vole~\mbox{%DIFAUXCMD
\cite{shafferStandardizedMultiomicsEarth2022} }\hskip0pt%DIFAUXCMD
(Qiita study ID 13114) for supplementary analyses.
}\DIFaddend 
  ...
  %\todo[noline]{R2.5}
  This is performed either by sub-sampling \DIFdelbegin \DIFdel{or by normalization }\DIFdelend of the matrix to \DIFdelbegin \DIFdel{the lowest sampling depth~\mbox{%DIFAUXCMD
\cite{Weiss2015}}\hskip0pt%DIFAUXCMD
.
  }\DIFdelend \DIFaddbegin \DIFadd{a specified rarefaction depth~\mbox{%DIFAUXCMD
\cite{weissNormalizationMicrobialDifferential2017} }\hskip0pt%DIFAUXCMD
in order to obtain samples with equal library sizes.
  }\todo[noline]{R3.5}
  \DIFaddend However, due to the potential biases and false positives~\cite{mcmurdieWasteNotWant2014,chaoCoveragebasedRarefactionExtrapolation2012} that might arise during the process, the rarefaction module is disabled by default and can be enabled in the configuration if needed.
  Hence, although the pipeline supports \DIFdelbegin \DIFdel{normalization and rarefaction, these modules are }\DIFdelend \DIFaddbegin \DIFadd{rarefaction, it is }\DIFaddend turned off by default\DIFdelbegin \DIFdel{, and }\DIFdelend \DIFaddbegin \DIFadd{.
  In addition to rarefaction, }\DIFaddend the \DIFdelbegin \DIFdel{analyses reported in the paper do not normalize or rareify the counts matrices.
  However, }\DIFdelend \DIFaddbegin \ac{micone} \DIFadd{pipeline also supports total sum scaling (TSS) and the centered log-ratio (clr) transformation (from the speiceasi R package).
  }\todo[noline]{R3.4}
  \DIFadd{However, since }\DIFaddend most of the network inference methods perform normalization and other transformation operations on the counts matrix as a part of their workflow\DIFaddbegin \DIFadd{, the analyses reported in the paper do not explicitly normalize the counts matrices}\DIFaddend .
  ...
  A null model is created by re-sampling and \DIFdelbegin \DIFdel{bootstrapping }\DIFdelend \DIFaddbegin \DIFadd{permuting }\DIFaddend the counts matrix and recalculating the correlations (see next section for details on network analysis and statistics).
  These \DIFdelbegin \DIFdel{bootstrapped }\DIFdelend \DIFaddbegin \DIFadd{permuted }\DIFaddend association matrices are used to calculate the significance of the inferred correlations by calculating the p-values against this null model~\cite{Watts2018}.
  ...
  The \DIFdelbegin \DIFdel{bootstrapping }\DIFdelend \DIFaddbegin \DIFadd{permutations }\DIFaddend and p-value calculations are only performed on the correlations-based methods.
 ...
 Note that all networks to be compared were updated to have the same number of nodes.
  ...
  \subsubsection*{\DIFdelbegin \DIFdel{Bootstrapping }\DIFdelend \DIFaddbegin \DIFadd{Permutations }\DIFaddend and p-value calculation}
  \vspace{-5mm}
  ...
  Note that, all the direct association-based methods used in the study have their own regularization methods built in and hence do not need to undergo \DIFdelbegin \DIFdel{bootstrapping}\DIFdelend \DIFaddbegin \DIFadd{this procedure}\DIFaddend .
  ...
  Both the algorithms make use of a user-defined parameter \DIFdelbegin \DIFdel{$p$ ($0 \leq p \leq 1$}\DIFdelend \DIFaddbegin \DIFadd{$\theta$ ($0 \leq \theta \leq 1$}\DIFaddend ), in order to threshold the edges from the individual methods.
  The inputs to both the algorithms are the co-occurrence networks (association matrices) $\bar{N}^i$ (flattened version of $N^{i}$) generated by each method $i$, and the threshold parameter \DIFdelbegin \DIFdel{$p$}\DIFdelend \DIFaddbegin \DIFadd{$\theta$}\DIFaddend .
...
where, $g$ and $f$ are defined as follows:
 ...
 The edge $j$ is selected to be present in the final consensus network if the number of networks in which $j$ appears is greater than a threshold, i.e:
  \begin{equation}
     M_j \geq \lfloor \DIFdelbegin \DIFdel{p }\DIFdelend \DIFaddbegin \DIFadd{\theta }\DIFaddend \times w \rfloor
    \label{eqn:simple-voting2}
  \end{equation}
  where, \DIFdelbegin \DIFdel{$p$ }\DIFdelend \DIFaddbegin \DIFadd{$\theta$ }\DIFaddend is the user-defined threshold parameter.

  The simple voting method returns the union of the networks when \DIFdelbegin \DIFdel{$0 \leq p \leq \frac{1}{w}$ }\DIFdelend \DIFaddbegin \DIFadd{$0 \leq \theta \leq \frac{1}{w}$ }\DIFaddend and will return the intersection when \DIFdelbegin \DIFdel{$\frac{(w - 1)}{w} \leq p \leq 1$}\DIFdelend \DIFaddbegin \DIFadd{$\frac{(w - 1)}{w} \leq \theta \leq 1$}\DIFaddend .
  In general, if \DIFdelbegin \DIFdel{$\frac{(n - 1)}{w} \leq p \leq \frac{n}{w}$}\DIFdelend \DIFaddbegin \DIFadd{$\frac{(n - 1)}{w} \leq \theta \leq \frac{n}{w}$}\DIFaddend , this algorithm will report an edge in the consensus network when at least $n$ network inference methods report this edge.
  ...
  \begin{equation}
    \mid s_j \mid > (w - 1) \times \DIFdelbegin \DIFdel{p
    }\DIFdelend \DIFaddbegin \DIFadd{\theta
    }\DIFaddend \label{eq:scaled-sum2}
  \end{equation}
  ...
  \DIFdelbegin \DIFdel{One }\DIFdelend \DIFaddbegin \DIFadd{A major }\DIFaddend issue with this approach is that the possibility of correlations existing between the edges of the network could lead to inaccurate estimates of the variance if a linear model were used to directly model the relationships between edges and steps in the workflow.
  \DIFdelbegin \DIFdel{In }\DIFdelend \DIFaddbegin \todo[noline]{R2.5}
  \DIFadd{Therefore, in }\DIFaddend order to remedy this issue, we performed a PCA (Principal Component Analysis) on the matrix $\mathbf{\bar{N}}$ to obtain the $\mathbf{C}$ matrix ($W \times c$) of components for each network, such that we reduce the dimensions from the $Q^2$ dimensional edge space to a $c$ dimensional component space.
  ...
  \vspace{-5mm}
  In Figure~\ref{fig:figure5} we evaluated the \DIFdelbegin \DIFdel{accuracy and robustness }\DIFdelend \DIFaddbegin \DIFadd{precision and sensitivity }\DIFaddend of the inferred association networks (using the various network inference algorithms and the consensus methods) against the original interaction network used to create the taxonomic distribution.
...
% Tables
  ...
  \begin{table}[H]
    ...
    \caption{
      ...
      The consensus algorithm in the Network Inference (\ac{ni}) step incorporates all the modules (\DIFdelbeginFL \DIFdelFL{bootstrapping}\DIFdelendFL \DIFaddbeginFL \DIFaddFL{permutations}\DIFaddendFL , direct association, and correlation-based) to generate the consensus network.
    }
...
% Figures
  ...
  \begin{figure}[H]
    ...
    \caption{
      ...
      The different points on the box plot show the precision \DIFaddbeginFL \DIFaddFL{(A, C) and sensitivity (B, D) }\DIFaddendFL of co-occurrence networks generated through individual network inference methods and \DIFdelbeginFL \DIFdelFL{through }\DIFdelendFL consensus network construction approaches.
      Precision \DIFdelbeginFL \DIFdelFL{is }\DIFdelendFL \DIFaddbeginFL \DIFaddFL{and sensitivity are }\DIFaddendFL estimated based on the comparisons with two sets of synthetic benchmark datasets (``NorTA'' and ``seqtime'', see Methods).
      The independent algorithms chosen for the comparison are the two best-performing correlation-based (propr, sparcc) and \DIFdelbeginFL \DIFdelFL{direct association based }\DIFdelendFL \DIFaddbeginFL \DIFaddFL{direct-association-based }\DIFaddendFL (spieceasi, flashweave) methods.
      For consensus network inference, we used the scaled-sum (SS) and simple voting (SV) methods.
      A weight threshold of 0.1 and a p-value threshold of 0.05 was applied to each network before the calculation of precision \DIFaddbeginFL \DIFaddFL{and sensitivity}\DIFaddendFL .
      The \DIFdelbeginFL \DIFdelFL{overall best }\DIFdelendFL \DIFaddbeginFL \DIFaddFL{purpose behind the construction of the consensus algorithms is to capture true associations in the data through the removal of associations that have a lower probability of being present in the networks inferred by different inference algorithms.
      Therefore, an increase in }\DIFaddendFL precision \DIFdelbeginFL \DIFdelFL{was consistently obtained }\DIFdelendFL \DIFaddbeginFL \DIFaddFL{is followed }\DIFaddendFL by \DIFdelbeginFL \DIFdelFL{the }\DIFdelendFL \DIFaddbeginFL \DIFaddFL{a decrease in sensitivity.
      The }\DIFaddendFL scaled-sum consensus method \DIFaddbeginFL \DIFaddFL{consistently obtained the overall best precision }\DIFaddendFL for \DIFdelbeginFL \DIFdelFL{$p \geq 0.333$ }\DIFdelendFL \DIFaddbeginFL \DIFaddFL{$\theta \geq 0.333$ }\DIFaddendFL on both benchmark datasets. Among all the individual network inference methods, spieceasi shows the best average precision.
      \DIFdelbeginFL \DIFdelFL{The simple voting method, when }\DIFdelendFL \DIFaddbeginFL \DIFaddFL{When }\DIFaddendFL using the presence of edges in all inferred networks as a requirement (\DIFdelbeginFL \DIFdelFL{$p = 1.000$}\DIFdelendFL \DIFaddbeginFL \DIFaddFL{$\theta = 1.000$}\DIFaddendFL ), \DIFaddbeginFL \DIFaddFL{the simple voting method }\DIFaddendFL also outperforms spieceasi on average precision.
      Therefore, we set the scaled-sum consensus method with \DIFdelbeginFL \DIFdelFL{$p = 0.333$ }\DIFdelendFL \DIFaddbeginFL \DIFaddFL{$\theta = 0.333$ }\DIFaddendFL as the default tool for consensus network inference, since this option provides a good balance of precision and sensitivity (see also Figure~\ref{fig:figure_s5} and \ref{fig:figure_s6}).
      \DIFaddbeginFL \DIFaddFL{The correlation-based methods (propr, and }\acs{sparcc}\DIFaddFL{) and the simple-voting consensus method return networks with higher sensitivities.
    }\DIFaddendFL }
    \label{fig:figure5}
  \end{figure}
  ...
  \begin{figure}[H]
    \centering
      \caption{
        ...
        The simple voting method when using the presence of edges in all inferred networks as a requirement (\DIFdelbeginFL \DIFdelFL{$p = 1.000$}\DIFdelendFL \DIFaddbeginFL \DIFaddFL{$\theta = 1.000$}\DIFaddendFL ), also outperforms \acs{spieceasi} on average precision (0.969).
  ...
  \begin{figure}[H]
    \centering
      \caption{
        ...
        The simple voting method when using the presence of edges in all inferred networks as a requirement (\DIFdelbeginFL \DIFdelFL{$p = 1.000$}\DIFdelendFL \DIFaddbeginFL \DIFaddFL{$\theta = 1.000$}\DIFaddendFL ), also outperforms \acs{spieceasi} on average precision (0.692).
  ...
  %DIF >  \begin{figure}[H]
  %DIF >    \centering
  %DIF >    \includegraphics[width=1.0\linewidth]{figure_s8.pdf}
  %DIF >  \end{figure}
  \DIFaddbegin \begin{figure}[H]
    \centering
    \caption{
      \textbf{\DIFaddFL{Network variance analysis of the stool samples from radiation-exposed bank vole (EMP dataset)}}\DIFaddFL{.
      }\textbf{\DIFaddFL{(A)}} \DIFaddFL{The percentage of variance in the networks contributed by the }\acf{dc}\DIFaddFL{, CC (chimera checking), }\acf{ta}\DIFaddFL{, }\acf{op} \DIFaddFL{and }\acf{ni} \DIFaddFL{steps of the pipeline calculated using ANOVA on a linear model (see Methods).
      A weight threshold of 0.1 and a p-value threshold of 0.05 were applied to each network before the analysis.
      The taxonomy database contributes most to the variance between the networks (57.96\%) followed by the filtering of the counts matrix (33.48\%) in the OP step.
    The variation due to the NI, DC and CC steps are much smaller in comparison (6.69\%, 1.30\%, and 0.00\% respectively).
      The negligible fraction labeled as the residual is an artifact that arises when multiple steps are changed at the same time.
      }\textbf{\DIFaddFL{(B)}} \DIFaddFL{All the inferred networks generated from various combinations of tools are shown as points on a PCA plot.
      ...
      The color of the points corresponds to the tools used at each step of the pipeline (DC, TA, OP, and NI).
      The points on the PCA plot can be largely grouped based on the TA step, and the extent of this separation decreases when the filtering is turned on in the OP step.
      However, unlike Figure~\ref{fig:figure6} B, here we observe that the networks that utilize }\ac{gg} \DIFaddFL{and }\ac{ncbi} \DIFaddFL{databases have the variation along similar axes in the 2 dimensional PCA plot.
      This could imply that the taxonomy assignments returned by these two databases for this dataset, might be more similar to each other compared to those in the FMT dataset.
    }}
    \label{fig:figure_s8}
  \end{figure}
  %DIF >  \FloatBarrier
  ...
  \begin{figure}[H]
    \centering
    \caption{Supplemental text describing data processing, synthetic interaction data generation, network metrics, p-value merging\DIFaddbeginFL \DIFaddFL{, additional network variance analysis }\DIFaddendFL and supplementary discussion sections.}
    \label{txt:text_s1}
  \end{figure}

%DIF <  \input{sections/supplementary.tex}
\DIFaddbegin \newpage
...
proposed that for $k$ independent p-values, each generated by $k$ different methods and denoted by $\bar{P}^i$ (notations are same as used in the "Consensus network and p-value merging" subsection of the Methods).
  %DIF > \todo[noline]{R1.1}
...
is then given by:
  ...
  \subsection*{\DIFadd{The JSON network format and network exports}}
    ...
    \DIFadd{The default format }\ac{micone} \DIFadd{uses for storing the network files is the JSON (JavaScript Object Notation) format which is supported by the }\acf{mind}\DIFadd{~\mbox{%DIFAUXCMD
...
for the export functionality, networks can be exported to all formats supported by the package.
  ...
  \subsection*{\DIFadd{Network variance analysis of stool samples from radiation-exposed bank vole (EMP dataset)}}

  \DIFadd{In order to verify the consistency of the network variance analysis, we processed an additional 16S rRNA dataset through the }\ac{micone} \DIFadd{pipeline.
  }\todo[noline]{R3.1}
  \DIFadd{Specifically, we used stool samples from radiation-exposed bank vole that were a part of the Earth Microbiome Project (EMP)~\mbox{%DIFAUXCMD
\cite{shafferStandardizedMultiomicsEarth2022}}\hskip0pt%DIFAUXCMD
.
  We chose a dataset from the EMP because the }\ac{micone} \DIFadd{pipeline inherently supports the EMP amplicon protocol with minimal pre-processing requirements.
  The data containing the 16S sequencing reads (V4 region) was downloaded from Qiita~\mbox{%DIFAUXCMD
\cite{gonzalezQiitaRapidWebenabled2018} }\hskip0pt%DIFAUXCMD
(study ID: 13114).
  For the analysis, the paired-end sequencing data extracted from stools samples of radiation-exposed bank vole (named ``mousseau'') were chosen from run ``EMP500\_6-9''.
  The data was then processed using the }\ac{micone} \DIFadd{pipeline starting at the }\ac{sp} \DIFadd{step and ending at the }\ac{ni} \DIFadd{step with the consensus algorithm.
  The configuration files (main.nf and nextflow.config) used to run the }\ac{micone} \DIFadd{pipeline as well the details of the pipeline execution (dag, report, timeline and trace) are in the "runs/EMP" directory of the data and scripts repository (}\href{https://github.com/segrelab/MiCoNE-pipeline-paper}{\DIFadd{https://github.com/segrelab/MiCoNE-pipeline-paper}}\DIFadd{)
}

  \DIFadd{Similar to the previous network variance analysis performed in Figure~\ref{fig:figure6}, we analyzed the effect of different processing methods on the inferred co-occurrence networks generated using all possible combinations of methods.
  Figure \ref{fig:figure_s8}A, shows the percentage of total variation among the co-occurrence networks due to the different steps of the pipeline.
  The }\ac{ta} \DIFadd{step, or more specifically the choice of 16S reference database, contributes the most ($57.96\%$) to the variation in the networks, followed by the }\ac{op} \DIFadd{step ($33.48\%$).
  These results are very similar to those from the network variance analysis of the FMT dataset (Figure~\ref{fig:figure6}A).
  This implies that the effects of various workflow steps on the inferred networks are fairly consistent across different datasets.
}

  \DIFadd{The effects of the different steps of the pipeline on the inferred networks can be visualized through dimensionality reduction.
  The PCA in Figure \ref{fig:figure_s8}B shows all possible inferred networks, colored by the tools used in the DC, TA, OP, and NI steps in each subfigure.
  We observe that the networks largely segregate based on the database used (TA step), and the extent of this separation decreases when the filtering is turned on in the OP step.
  However, unlike the case of the FMT dataset (Figure~\ref{fig:figure6}B), here we observe that the networks that utilize }\ac{gg} \DIFadd{and }\ac{ncbi} \DIFadd{databases have the variation along similar axes in the 2 dimensional PCA plot.
  This could imply that the taxonomy assignments returned by these two databases for this dataset, might be more similar to each other compared to those in the FMT dataset.
  The variation in the networks due to the other workflow steps is consistent with that observed in the FMT data analysis (Figure~\ref{fig:figure6}B).
}
...
in taxonomy hierarchies in these databases.
  ...
  %DIF > \todo[noline]{R2.12}
  The default reference database in the pipeline is currently chosen to be the }\ac{gg} \DIFadd{reference database along with the ``Naive Bayes'' classifier as the query tool.
...
in taxonomic studies, which would enable easy comparison across datasets.
  ...
  \DIFadd{However, we recommend using the SILVA database for newer studies due to its size and taxonomic comprehensiveness~\mbox{%DIFAUXCMD
...
and since }\ac{gg} \DIFadd{has not been updated since 2013.
  ...
  For example, to generate a dataset that is compatible with the }\ac{mind} \DIFadd{platform~\mbox{%DIFAUXCMD
\cite{huResourceComparisonIntegration2022}}\hskip0pt%DIFAUXCMD
, }\ac{ncbi}\DIFadd{, is the most appropriate choice as it guarantees compatibility of taxonomic hierarchy and therefore comparability with other datasets.
  A detailed study of other taxonomy mapping approaches~\mbox{%DIFAUXCMD
\cite{muraliIDTAXANovelApproach2018,matiasrodriguesMAPseqHighlyEfficient2017} }\hskip0pt%DIFAUXCMD
and integrative approaches combining different databases~\mbox{%DIFAUXCMD
\cite{gwakDataDrivenModelingSpeciesLevel2020} }\hskip0pt%DIFAUXCMD
might offer orthogonal information and better matches.
  }\todo[noline]{R2.5a}
  \DIFadd{Furthermore, we also enable users to use custom databases~\mbox{%DIFAUXCMD
...
with the BLAST and Naive Bayes classifiers that are incorporated into the pipeline (from }\ac{qiime2}\DIFadd{).
  We suggest that the choice of the database should be made based on possible reported or inferred biases in the representation of given biomes in a specific databases~\mbox{%DIFAUXCMD
...
.
  ...
  We also observe that the reduction in the number of taxa leads to a better agreement in the networks inferred through different methods (Figure~\ref{fig:figure6} and Figure~\ref{fig:figure_s1}).
  }\todo[noline]{R2.3}
  \DIFadd{Moreover, filtering is necessary in order to increase the power in tests of significance when the number of taxa is much greater than the number of samples.
}

%DIF >  \newpage
%DIF >    \pagenumbering{goggle}
%DIF >    \thispagestyle{empty}
%DIF >    \input{table1.tex}
%DIF >    \thispagestyle{empty}
%DIF >    \input{table_s1.tex}
\DIFaddend 
